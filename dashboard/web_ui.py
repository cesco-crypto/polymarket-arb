...
async def api_safeguard_status() -> dict:
    ...
    return {
        ...
        "config": {
            ...
            "min_ev_threshold": strat.guards.min_ev_threshold,
            "max_drift_pct": strat.guards.max_drift_pct,
            "taker_fee_rate": strat.guards.taker_fee_rate,
...
_SAFEGUARD_DEFAULTS = {
    ...
    "min_ev_threshold": 0.04, "max_drift_pct": 50.0, "taker_fee_rate": 0.018,
    "max_concurrent": 10, "min_seconds_to_copy": 5, "poll_interval_s": 1.5,
...
async def api_safeguard_config(body: dict) -> dict:
    ...
    param_map = {
        ...
        "min_ev_threshold": (lambda: strat.guards.min_ev_threshold, lambda v: setattr(strat.guards, "min_ev_threshold", v), 0.01, 0.20, float),
        "max_drift_pct": (lambda: strat.guards.max_drift_pct, lambda v: setattr(strat.guards, "max_drift_pct", v), 10.0, 100.0, float),
        "taker_fee_rate": (lambda: strat.guards.taker_fee_rate, lambda v: setattr(strat.guards, "taker_fee_rate", v), 0.005, 0.05, float),
